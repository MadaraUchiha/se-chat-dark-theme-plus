...
const writeToSheet = obj => {
    userColorSheet.textContent += `div.user-${obj.key} .messages { border-top: solid .25em ${obj.color} !important; } `;
};
const colorUserParser = node => {
    const userColours = options.user_color_bars;
...
DOMObserver.addParser(colorUserParser, '.user-container');